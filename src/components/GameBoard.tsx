...
export function GameBoard({ boardMap, pathCoords, players, currentTurn }: GameBoardProps) {
  ...
  return (
    <div className="board-glow w-full max-w-[min(54dvh,640px)] aspect-square relative">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {players.map((player) => {
          ...
          return (
            ...
            >
              <div 
                className={`relative flex items-center justify-center w-8 h-8 sm:w-11 sm:h-11 rounded-full shadow-lg transition-transform duration-300 ${isActive ? 'avatar-pulse scale-110' : ''}`}
                style={{ 
